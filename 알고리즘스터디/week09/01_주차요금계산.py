...
def solution(fees, records):
    base_time,basic_fee,unit_time,unit_fee = fees
    db = defaultdict(list)
    db_time = defaultdict(list)
    db_fee = defaultdict(list)
    ...
    for car_num, times in db_time.items():
        ...
        if total_time < base_time:
            db_fee[car_num] = basic_fee
        else:
            db_fee[car_num] = basic_fee + math.ceil((total_time-base_time)/unit_time) * unit_fee
            
    return [db_fee[key] for key in sorted(db_fee.keys())]	
